...
case class Handle(serial:Long, handle:Ptr[Byte]) {
  ...
  def stream(itemHandler:StreamIO.ItemHandler,
             doneHandler:StreamIO.DoneHandler):Unit = {
  ...
  def streamUntilDone(handler:StreamIO.ItemHandler):Future[Long] = {
    ...
    val itemHandler:StreamIO.ItemHandler = (data,handle,id) => try {
      handler(data,handle,id)
    } catch {
      case t:Throwable => promise.failure(t)
    }

    val doneHandler:StreamIO.DoneHandler = (handle,id) =>
      promise.success(id)
