...
object Curl {
  ...
  def startRequest(method:Int, url:String,headers:Seq[String] = Seq.empty,body:String = ""):Future[ResponseState] = Zone { implicit z =>
    init()
    ...
    method match {
      case GET =>
        ...
        check(easy_setopt_ptr(curlHandle, WRITECALLBACK, func_to_ptr(dataCB)),"easy_setopt")
        check(easy_setopt_ptr(curlHandle, WRITEDATA, req_id_ptr.asInstanceOf[Ptr[Byte]]),"easy_setopt")
        check(easy_setopt_ptr(curlHandle, HEADERCALLBACK, func_to_ptr(headerCB)),"easy_setopt")
        check(easy_setopt_ptr(curlHandle, HEADERDATA, req_id_ptr.asInstanceOf[Ptr[Byte]]),"easy_setopt")
        check(easy_setopt_ptr(curlHandle, PRIVATEDATA, req_id_ptr.asInstanceOf[Ptr[Byte]]),"easy_setopt")
      case POST =>
        // TODO
  ...
  def multi_setopt(curl:MultiCurl,option: CInt, parameters:CVarArg*):Int = Zone { implicit z =>
...
@extern object LibCurl {    
  ...
  @name("curl_easy_setopt")
  def easy_setopt_ptr(handle: Curl, option: CInt, parameter: Ptr[Byte]): CInt = extern
